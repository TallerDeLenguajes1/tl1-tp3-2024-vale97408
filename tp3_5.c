...
int main(){
  ...
  scanf("%d",&cantN );
    ...
    char **nombres = (char**)malloc(cantN* sizeof(char*)); // Doble puntero para manejar un conjunto dinámico de punteros a cadenas de caracteres
    ...
    {
        printf("Nombre Nro %d: \n", i+1);
        fflush(stdin); // Limpio buffer de entrada para evitar problemas de lectura
